...
- **Login**: Supabase Auth, e-mail/senha OU "Entrar com Google" (desde 2026-09-11, só contas @legadodigital.net) — papéis `Admin Legado Digital` e `Operador Legado Digital`.
- **Dashboard** (`/admin`): cards de Parceiros/Memoriais/Usuários, métricas de visita (total acumulado, novos memoriais 7 dias, homenagens recentes), top 5 cemitérios e top 5 parceiros por visita, card de e-mail do fornecedor de placas, tabela de memoriais com QR Code. Header (canto superior direito, visível em toda a Central): dropdown "Parceiros" pra pular direto pro Portal de qualquer parceiro sem abrir a ficha, sino de alertas com o feed de e-mails disparados (`emails_enviados` — mesma fonte de `/admin/emails`, cada item linka pro memorial ou pra Central de Comunicações) **mais os leads novos ainda não lidos**, que aparecem no topo do dropdown; cada e-mail mostra bolinha verde (enviado) ou vermelha (erro), e no erro o motivo (`erro_msg`) aparece direto na lista, sem precisar abrir a tela cheia (2026-09-10); badge vermelho quando algum e-mail falhou, âmbar quando só há lead esperando resposta, e toggle de tema claro/escuro que recolore a Central inteira E o Portal do Parceiro (preferência única, mesma pessoa cruza os dois) — dark é o padrão, claro é opcional. Landing/Portal da Família/página pública do memorial nunca são afetados pelo toggle.
- **Parceiros** (`/admin/parceiros`): CRUD completo. A ficha de detalhe (`/admin/parceiros/[id]`) virou **abas** em 2026-09-16, mesma estrutura do Portal da Família — cabeçalho fixo (nome, tipo+cidade, badge Ativo/Inativo, botão "Acessar Plataforma do Parceiro") acima de 5 abas na ordem de prioridade: **Dados** (razão social/CNPJ/e-mail/telefone — só leitura, sem formulário de edição ainda), **Contatos e Acesso** (lista de contatos com nome/e-mail/telefone/perfil — Responsável Legal/Financeiro/Comercial/Técnico/Outro — e "Conceder acesso" por contato, mais "Conceder acesso avulso" pra quem não está na lista), **Página Pública** (logo, descrição institucional), **Memoriais** (tabela, só leitura) e **Plano** (plano contratado, status de pagamento, desde quando).
- **Cemitérios** (`/admin/cemiterios`): cadastro com mapa (satélite Esri) pra marcar localização, botão "Instalação Drone" (relatório técnico de mapeamento por drone). Cada cemitério tem dois links próprios: **Mapa** (`/admin/cemiterios/[id]/mapa`) e **Lápides** (`/admin/cemiterios/[id]/lapides`). Toggle "Visível em /cemiterios" (2026-08-13) controla se aquele cemitério entra no **diretório público sem login** (`/cemiterios` → cidade → cemitério → mapa com pino de cruz nos túmulos que têm memorial publicado) — só a Central liga/desliga, Parceiro só vê o chip de leitura.
  - **Mapa**: sistema de endereçamento Quadra → Fileira → Túmulo sobre o ortomosaico real do drone (quando o cemitério tem um cadastrado — José Lázaro/Tupaciguara e São Pedro/Uberlândia têm hoje, 2026-08-13; os outros caem no satélite genérico). Passo a passo real: 1) botão "+ Nova quadra" desenha o contorno clicando os pontos; 2) dentro da quadra, "+ Nova fileira" desenha o eixo da fileira (clica início e fim); 3) ao concluir a fileira abre o card "Gerar túmulos" no topo do painel — digita a quantidade do lote e bolinhas verdes de preview aparecem na hora sobre a fileira, arrasta cada uma pro centro real do túmulo, "Gerar" salva e "Gerar mais" continua de onde parou; 4) pode duplicar uma fileira pronta pra virar a próxima (mesmo formato, deslocado — sempre nasce não revisada, precisa conferir antes de travar). Cada quadra/fileira pode ser travada (🔒) depois de revisada, o que bloqueia edição acidental. Quadra pode ser renomeada livremente (nome real do cemitério, ex: "Alameda dos Ipês", independente do número sequencial de descoberta) e também **renumerada** ("# Renumerar quadra") pro número real que o cemitério usa — isso regera o código de todos os túmulos dela (`Q02-R01-T005` vira `Q36-R01-T005`), troca de lugar com a quadra que já tinha aquele número se houver, e pede confirmação se algum túmulo afetado já tem memorial vinculado. **Cadastro de memorial direto no túmulo (2026-08-14):** clicar com o **botão direito** num pino dá 2 opções — **"Criar memorial aqui"** (exige só o nome completo, nasce vinculado àquele túmulo, abre a ficha completa depois e aparece na lista de Memoriais como qualquer outro) e **"Vincular memorial existente"** (busca por nome no cadastro; se o memorial já estiver em outro túmulo, avisa que vai mover, não duplica); botão direito num ponto vazio marca um túmulo avulso ali. **Card do jazigo (revisto 2026-09-15):** o clique normal no pino abre um card **só quando aquele jazigo já tem memorial** — túmulo vazio não abre card nenhum (criar memorial ali é pelo botão direito). O card mostra o **nome do jazigo** no topo (`lapides.nome`, ex: "Jazigo Família Saraiva"; sem nome cai no código técnico) e, embaixo, **quem está em cada gaveta**: `G1`, `G2`… com a foto e o nome completo de quem tem memorial (clicável, leva pra **página pública** do memorial, `/homenagem/[slug]`, não pra ficha de edição) e só o nome, como texto, de quem ainda não tem memorial digital. No card dá pra anexar/tirar a **foto da lápide**, desvincular, vincular memorial existente e mover o pino — o resto da edição (cadastrar gaveta, renomear o jazigo, escrever quem está numa gaveta sem memorial) fica na **página do jazigo**, linkada do próprio card ("Abrir página do jazigo →"). Neste mapa **não existe mais card de passar o mouse** — é tela de trabalho, o card de clique já mostra tudo; o card pequeno de hover continua no Portal da Família e no mapa público. Memorial criado assim fica marcado como `origem_cadastro = 'mapa_cemiterio'` e, se tiver dado faltando, aparece no **alerta do Dashboard da Central** ("N memoriais com dados faltando", com o que falta em cada um). "Marcar túmulo avulso" serve pra lápide que já existe no cadastro mas ainda não tem coordenada (fora do fluxo quadra/fileira). A **foto do túmulo** tirada de perto (`lapides.foto_face_url`) se sobe pelo card do jazigo (quando já tem memorial) ou pela página do jazigo — foto aérea de drone é reta de cima, nunca mostra o nome gravado na pedra, então a foto de campo é o que identifica. **Subir a foto do túmulo marca ele como conferido em campo** — o pino fica **verde** no mapa e, na tela de Lápides, o quadradinho dele ganha contorno sólido + bolinha verde (o contador da fileira mostra "N de M conferidos"). É a regra do sistema: foto de perto só existe se alguém esteve lá fisicamente. **Conferir com GPS de campo** (2026-08-17): staff cola no painel do mapa o link do Google Maps (ou os números) que alguém mandou de dentro do cemitério — cai um pino laranja e o painel mostra qual túmulo está mais perto e o desvio em metros, com o passo a passo do procedimento de campo escrito ali mesmo. Dá pra guardar como referência do túmulo (não sobrescreve a coordenada do ortomosaico) ou mover o túmulo pra lá (com aviso: ortomosaico é mais preciso que GPS de celular). **Pontos de referência** (Capela, Administração, Sanitários, Velório) marcam as âncoras físicas do mapa impresso da prefeitura — pino roxo, arrasta pra ajustar e **trava** (🔒) quando estiver no lugar certo; servem pra conferir se a numeração oficial de quadra bate com a geometria real. "Ruas (caminhos internos)" desenha o caminho real por onde a pessoa anda — a rota do "Como Chegar" público passa a seguir esse caminho em vez de linha reta. Painel "Quem opera neste cemitério" (só staff) mostra os memoriais agrupados por parceiro dono. No painel de Quadras, embaixo de cada fileira, aparece a lista dos túmulos que já têm memorial no formato `Túmulo 11 — Nome do memorial`, com link direto pra ficha (2026-08-17).
  ...
  - **Lápides**: visualização organizada por Quadra (colunas lado a lado) → Fileira (retrátil) → Túmulo (chip clicável, mostra se já tem memorial). Embaixo dos chips de cada fileira sai a lista `Túmulo N — Nome do memorial` (só os que têm memorial), com link pra ficha. Túmulos sem quadra/fileira vinculada ficam numa seção separada "Fora de fileira", com aviso — vincular memorial num túmulo dessa seção exige confirmação extra (risco de errar o túmulo certo).
...
- **Dashboard**: total de memoriais, plano contratado, status de pagamento, edição da própria página pública (logo/descrição — corrigido 2026-07-29, RLS bloqueava o salvar antes). O campo de logo tem **"Remover logo"** desde 2026-08-26, nos dois lados (Central e Portal do Parceiro): remove de verdade, apaga o arquivo do servidor. Até essa data o logo não aparecia em tela nenhuma (o balde virou privado em 18/08 e a URL continuou pública) e não havia como tirar — só sobrescrever.
...
- **A tela é organizada em abas (2026-09-16):** `Memoriais · Fotos e vídeos · Jazigo · Árvore · Privacidade · Assinaturas`, no topo. Cada aba abre sozinha na tela inteira — antes era tudo empilhado numa página sem fim. Se a família disser "não acho onde muda X", oriente pela aba, não por rolagem.
- **O topo mostra a família, não o homenageado:** "PORTAL DA FAMÍLIA" + o nome do jazigo (ex: "Jazigo Família Saraiva"). Enquanto ninguém nomeou o jazigo, aparece "Família de [nome do homenageado]". A aba Memoriais lista todos os memoriais daquele jazigo, com retrato redondo — o que ela está editando fica em destaque, os outros levam pra página pública deles.
- **A família agora edita o jazigo** (antes era só ver): pode dar nome ao jazigo e escrever o nome de quem está enterrado nas gavetas **que ainda não têm memorial**. O que continua sendo só do parceiro/staff: **criar e remover gaveta**, e mexer em gaveta que já tem memorial (pode ser de outra família). O campo "Família (pode deixar em branco)" no formulário grava exatamente o mesmo nome do jazigo — são dois lugares de editar a mesma coisa, não dois nomes diferentes.
- **Modo ajuda:** botão no topo da página. Ligado, aparece um "?" dourado do lado de cada campo; passar o mouse (ou tocar) mostra uma explicação curta de pra que serve aquele campo. Desligado, a tela fica limpa. A escolha fica guardada no navegador dela. Se a família estiver perdida nos campos, mande ligar o Modo ajuda.
- **Assinaturas:** a aba mostra só a lista de quem assinou, com "remover" do lado — o livro desenhado continua na página pública do memorial, aqui não é redesenhado.
...
- **Se a família escondeu o próprio memorial e não acha ele no login** (2026-09-16): memorial com privacidade "oculto" ou com a busca desligada não aparece na busca da tela de login — é de propósito, senão esconder não significaria nada. Nesse caso a tela oferece, logo abaixo do "nenhum memorial encontrado", um campo pra ela **digitar a senha ali mesmo** e entrar pelo nome que ela já digitou. Oriente por esse caminho. Se ela errar o nome ou a senha, a mensagem é sempre a mesma — o sistema nunca confirma se um memorial escondido existe.
...
- **Se a família reclamar do aviso "recuperei o que você tinha escrito e ainda não estava salvo" sem ter editado nada:** era um bug corrigido em 16/09 — a própria abertura da página gravava um rascunho igual ao que já estava salvo. Já está corrigido, mas o rascunho falso que ficou guardado no navegador dela ainda aparece **uma última vez**: mande clicar em "Descartar e usar a versão salva" e não volta mais.
...
## Captação de leads (desde 2026-09-04)

As telas públicas de acesso são divididas ao meio: metade clara com um formulário de contato (à esquerda no desktop; no celular o login aparece primeiro) e metade navy com o login de sempre.

- `/parceiro/login` — formulário "Leve o Legado Digital pra sua funerária": nome do responsável, empresa, e-mail, telefone, cidade. Vira **lead de parceiro**.
- `/familia/login` — formulário "Quero um memorial pra minha família": nome, e-mail, telefone, cidade, nome do homenageado (opcional). Vira **lead de família**, que a equipe encaminha pra uma funerária parceira da região. A família **nunca se cadastra sozinha pra ter acesso** — quem cria o memorial e envia a senha é a funerária ou a Central.
