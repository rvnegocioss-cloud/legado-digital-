...
import { getParceiroUser } from '@/lib/auth'
...
export async function POST(req: NextRequest) {
  // 1. Autenticação (parceiro B2B)
  const user = await getParceiroUser()
  if (!user) {
  ...
  // 2. Validar memorial e verificar se parceiro é o dono
  ...
  // Verificar se o parceiro logado é o dono do memorial
  const parceiroId = user.parceiros_usuarios?.[0]?.parceiro_id
  if (homenagem.parceiro_id !== parceiroId) {
