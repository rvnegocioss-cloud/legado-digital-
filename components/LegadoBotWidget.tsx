...
export default function LegadoBotWidget() {
  const [aberto, setAberto] = useState(false)
  const [mensagens, setMensagens] = useState<Mensagem[]>([
    { role: 'assistant', content: 'Oi, eu sou o LegadoBot. Pode perguntar sobre qualquer parte do sistema.' },
  ])
  ...
  async function enviar() {
    ...
    try {
      ...
      if (!token) {
        setMensagens([...novasMensagens, { role: 'assistant', content: 'Sessão expirou, atualize a página e entre de novo.' }])
        return
      }

      const resp = await fetch('/api/legadobot/chat', {
        method: 'POST',
      ...
      setMensagens([...novasMensagens, { role: 'assistant', content: 'Não consegui conectar. Confirme que a freellmapi está rodando local.' }])
    } finally {
      setCarregando(false)
    }
  }
