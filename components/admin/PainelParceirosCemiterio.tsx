...
export default function PainelParceirosCemiterio({ cemiterioId }: { cemiterioId: string }) {
  ...
  if (grupos.length === 0) {
    return (
      <div className="rounded-xl bg-[var(--tema-zinc-900)] border border-[var(--tema-zinc-800)] p-4 mt-4">
        ...
        <p className="text-xs text-[var(--tema-zinc-500)]">Nenhum memorial cadastrado ainda.</p>
      </div>
    )
  }
  ...
  return (
    <div className="rounded-xl bg-[var(--tema-zinc-900)] border border-[var(--tema-zinc-800)] p-4 mt-4">
      <h2 className="text-sm font-semibold text-white mb-1">Quem opera neste cemitério</h2>
      <p className="text-xs text-[var(--tema-zinc-500)] mb-3">Memoriais agrupados por parceiro dono -- útil quando mais de uma funerária atende o mesmo cemitério.</p>
      <div className="space-y-1.5">
        {grupos.map((g) => {
          const chave = g.parceiro_id || '__sem_parceiro__'
          const aberto = expandido === chave
          return (
            <div key={chave} className="rounded border border-[var(--tema-zinc-800)]">
              <button
                type="button"
                onClick={() => alternar(chave, g.parceiro_id)}
                className="w-full flex items-center gap-1.5 text-left text-sm px-3 py-2 text-[var(--tema-zinc-200)] hover:bg-[var(--tema-zinc-800)]"
              >
                {aberto ? <ChevronDown size={14} /> : <ChevronRight size={14} />}
                {g.parceiro_nome || 'Central / sem parceiro'}
                <span className="text-[var(--tema-zinc-500)] text-xs ml-auto">
                  ...
                  {carregandoGrupo === chave ? (
                    ...
                    <ul className="space-y-1">
                      {(memoriaisPorGrupo[chave] || []).map((m) => (
                        <li key={m.homenagem_id} className="text-xs flex items-center justify-between text-[var(--tema-zinc-300)]">
      ...
      <Link href={`/admin/cemiterios/${cemiterioId}/lapides`} className="text-xs text-[var(--tema-zinc-500)] hover:text-[var(--tema-zinc-300)] inline-block mt-3">
