...
import { SelecaoJazigo, type JazigoEscolhido } from '@/components/admin/SelecaoJazigo'
...
// Ficha única do memorial (2026-09-16): a Central e o Portal do Parceiro usam
// exatamente esta tela. O que muda entre os dois é só quem pode abrir
// (parceiro só o próprio memorial, regra 22) e pra onde o "Voltar" leva.
export type ModoFicha = 'central' | 'parceiro'

export function FichaMemorial({ modo }: { modo: ModoFicha }) {
  return (
    <Suspense fallback={<p className="text-[var(--tema-zinc-400)]">Carregando...</p>}>
      <FichaMemorialInner modo={modo} />
    </Suspense>
  )
}

function FichaMemorialInner({ modo }: { modo: ModoFicha }) {
  const params = useParams<{ id: string }>()
  const router = useRouter()
  const searchParams = useSearchParams()
  const parceiroIdParam = modo === 'parceiro' ? searchParams.get('parceiro_id') : null
  const suffix = parceiroIdParam ? `?parceiro_id=${parceiroIdParam}` : ''

  const [memorial, setMemorial] = useState<Memorial | null>(null)
  // Presença ao vivo: avisa quando outra pessoa (família, funerária ou
  // Central) está editando o mesmo memorial agora. O papel real é decidido no
  // servidor pela credencial.
  const { outros: outrosEditando, temOutroEditando } = useTravaEdicao(memorial?.slug, undefined, modo === 'central' ? 'staff' : 'parceiro')
  const [nomeParceiroVinculado, setNomeParceiroVinculado] = useState<string | null>(null)
  const [acessandoFamilia, setAcessandoFamilia] = useState(false)
  const [aba, setAba] = useState<'familia' | 'memorial' | 'qr' | 'fotos' | 'privacidade' | 'moderacao'>('familia')
  ...
  const [jazigoEscolhido, setJazigoEscolhido] = useState<JazigoEscolhido | null>(null)
  ...
  async function load(id: string) {
    ...
    // Na Central (staff) não há trava de dono: o layout de /admin já barra
    // quem não é staff.
    ...
    if (!m || (modo === 'parceiro' && (!meuParceiroId || m.parceiro_id !== meuParceiroId))) {
      setMemorial(null)
      setLoading(false)
      return
    }

    if (modo === 'central' && m.parceiro_id) {
      const { data: p } = await supabase
        .from('parceiros_b2b')
        .select('nome_fantasia, razao_social')
        .eq('id', m.parceiro_id)
        .maybeSingle()
      setNomeParceiroVinculado(p ? p.nome_fantasia || p.razao_social : null)
    }
    ...
    if (m.lapide_id) {
      const { data: lapideAtual } = await supabase
        ...
        .eq('id', m.lapide_id)
        .maybeSingle()
      if (lapideAtual?.cemiterio_id) setCemiterioSelecionadoId(lapideAtual.cemiterio_id)
    }
    ...
    if (modo === 'central' && !m.qr_code_url && m.slug && nomeDoBanco && nomeDoBanco !== 'Novo memorial' && (m.data_falecimento || '').trim()) {
      // Central: auto-gera só se faltava (conserta falha anterior) — nunca num
      // rascunho sem nome/data reais, senão dispara pedido em branco pro
      // fornecedor da placa só de abrir a ficha.
  ...
  async function salvar(e: React.FormEvent) {
    ...
    if (form.lapide_id && form.lapide_id !== memorial.lapide_id) {
      const arriscado = jazigoEscolhido && (!jazigoEscolhido.fila_id || jazigoEscolhido.totalMemoriais > 0)
  ...
  return (
    <div>
      <Link
        href={modo === 'central' ? '/admin/memoriais' : `/parceiro/memoriais${suffix}`}
        className="inline-flex items-center gap-1.5 text-sm text-[var(--tema-zinc-400)] hover:text-white"
      >
        <ArrowLeft size={14} strokeWidth={1.5} />
        {modo === 'central' ? 'Voltar pra Memoriais' : 'Voltar pra Meus Memoriais'}
      </Link>

      <div className="flex flex-wrap items-start justify-between mt-4 mb-2 gap-4">
        <div className="min-w-0">
          <h1 className="text-2xl font-bold text-white">{memorial.nome_completo}</h1>
          {modo === 'central' && (
            <p className="text-[var(--tema-zinc-400)] text-sm mt-1">
              {nomeParceiroVinculado ? `Vinculado a ${nomeParceiroVinculado}` : 'Sem parceiro vinculado'}
            </p>
          )}
        </div>
        {memorial.slug && (
          <div className="flex flex-wrap gap-2 shrink-0">
            ...
            <a
              ...
              className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-blue-600 hover:bg-blue-700 text-branco-fixo text-sm font-medium whitespace-nowrap"
            >
      ...
      {temOutroEditando && (
        <div className="mt-4 rounded-lg border border-red-900/50 bg-red-950/30 px-4 py-3">
          ...
          <p className="text-xs text-red-300/80 mt-1">
            Evite salvar por cima — combine antes, ou espere terminar.
          </p>
        </div>
      )}
        ...
        <div className="@container">
          <div className="rounded-xl bg-[var(--tema-zinc-900)] border border-[var(--tema-zinc-800)] p-6">
            <form onSubmit={salvar}>
              ...
              <SecaoFicha titulo="Identificação" icon={User}>
                <div className="flex flex-wrap items-start gap-4">
                  ...
                  <CampoFicha label="Jazigo" className="w-48">
                    <SelecaoJazigo
                      cemiterioId={cemiterioSelecionadoId}
                      valorId={form.lapide_id}
                      onEscolher={(j) => {
                        setForm({ ...form, lapide_id: j?.id || '' })
                        setJazigoEscolhido(j)
                ...
                {(() => {
                  ...
                  const l = jazigoEscolhido
                  if (!l) return null
                  const arriscado = !l.fila_id || l.totalMemoriais > 0
                  ...
                  return (
                    <div className="rounded-lg bg-red-950/30 border border-red-900/40 px-3 py-2 mt-3 space-y-2">
                      <p className="text-xs text-red-300">
                        ...
                        {l.totalMemoriais > 0 && `Esse túmulo já tem ${l.totalMemoriais} memorial(is) vinculado(s). `}
      ...
      <div className={aba === 'familia' ? '' : 'hidden'}>
          <div className="rounded-xl bg-[var(--tema-zinc-900)] border border-[var(--tema-zinc-800)] p-6 max-w-2xl">
            <SecaoFicha titulo="Acesso da família" icon={Mail} primeira>
