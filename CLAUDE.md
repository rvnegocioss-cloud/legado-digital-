...
## Regra — Qual modelo usar pra cada tarefa (2026-07-15)
A ferramenta de subagente (Agent tool) aceita 4 modelos: `sonnet`, `opus`, `haiku`, `fable`. Política adotada — o que é **validado** vs **suposição a testar** fica marcado explicitamente, nunca tratar suposição como fato:

- **Opus — planeja tarefa complexa** (`subagent_type: "Plan"`, `model: "opus"`, só pesquisa/lê, não edita). Pra redesign grande, bug difícil de diagnosticar, feature nova com várias peças de arquitetura. **Validado 2x** nessa sessão: plano do redesign da página do memorial, e diagnóstico do bug real da chama da vela (colisão `transform`×`@keyframes`) — os dois vieram bem mais precisos que tentativa direta sem consultar antes.
- **Sonnet — executa** (código, edição de arquivo, build, commit). É o modelo padrão dessa sessão — não precisa spawnar subagente pra isso, é quem já está trabalhando.
- **Haiku — tarefa mecânica/simples de alto volume** (busca pontual, formatação, tarefa repetitiva onde velocidade/custo importa mais que profundidade de raciocínio). **Ainda não usado** nesse projeto — candidato certo quando aparecer esse tipo de tarefa, não testado ainda.
- **Fable 5 — não confirmado ainda.** Não tenho benchmark real de que ele planeja melhor que Opus pra arquitetura técnica — o nome sugere força em texto/narrativa, mas isso é suposição, não fato. **Antes de usar pra decisão técnica, testar primeiro em algo de baixo risco** (ex: revisar copy da landing, tom do LegadoBot) e comparar resultado real.

Regra geral: nunca inventar arquitetura complexa sozinho sem consultar Opus primeiro (regra permanente, validada). Escolha de modelo pras outras camadas (Haiku/Fable) é caso a caso, sem "escada" fixa até ter mais uso real acumulado.
