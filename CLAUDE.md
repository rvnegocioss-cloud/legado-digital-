...
- [x] **Modos de privacidade completos** (2026-07-31, planejado com Opus + executado no mesmo dia) — os 5 modos agora existem de verdade: aberto, com senha (já existia), com identificação (cadastro), lista de e-mails autorizados (email, allowlist + OTP de 6 dígitos por 10min), e oculto (some de busca/link/QR pra todo mundo, staff/parceiro/família continuam vendo pelos próprios painéis). Achado no meio do planejamento: já existia um scaffold de schema pra isso desde 2026-07-17 (coluna `modo_gate` + tabelas `memorial_visitantes`/`homenagens_emails_autorizados`/`memorial_email_codigos`) aplicado via MCP mas nunca salvo em `supabase/migrations/` nem usado em código — recuperado retroativamente (`20260717190749_...sql`). Arquitetura: 2 eixos ortogonais — canal (busca/link/QR, já existia) × portão (`modo_gate`), resolvidos em ordem fixa por `lib/modosPrivacidade.ts` (`resolverAcesso`): oculto vence tudo → canal errado responde igual a "não encontrado" (nunca "acesso restrito", anti-enumeração) → portão aberto passa → senha/cadastro/email pedem prova. Cookie de acesso (`lib/acessoMemorialSessao.ts`) ganhou `modo` + `gate_versao` no payload — trocar o modo ou salvar privacidade deriva `gate_versao+1` e derruba na hora qualquer cookie emitido antes, mesmo dentro da validade (30 dias pra senha, 7 pra cadastro/email). UI: componente único `components/admin/PrivacidadeMemorial.tsx` reaproveitado por Central e Portal do Parceiro (radio dos 5 modos + os 3 toggles de canal + gerenciador de e-mails autorizados quando modo=email + lista de visitantes identificados quando modo=cadastro) — elimina a duplicação que existia antes entre os dois lados. Rotas novas: `/api/memorial-acesso-cadastro`, `/api/memorial-acesso-email/solicitar`+`/confirmar`, `/api/memorial-emails-autorizados` (CRUD da lista), `/api/memorial-visitantes` (leitura); as 2 rotas de e-mail caem automaticamente no rate limit de login (3/min) do `proxy.ts` por prefixo de path (`/api/memorial-acesso*`), sem precisar mexer no proxy. OTP com rate limit no banco (não no Map em memória do `proxy.ts`, que não é compartilhado entre instâncias serverless da Vercel): 10/hora por memorial, 3/hora por e-mail, 5 tentativas erradas invalida o código, código expira em 10min, resposta sempre genérica (autorizado ou não) — mesmo padrão anti-enumeração de `/api/familia-esqueci-senha`. **Pendente, não construído ainda:**
- QR Code assinado (Fase 7 do plano) — hoje `qrcode_habilitado` continua combinado com `link_habilitado` (mesma aproximação de antes), então esse toggle sozinho ainda não distingue os 2 canais; se a família desligar só QR ou só link, os dois caem juntos. Motivo de adiar: mudar o QR embutido invalidaria placa física já impressa — hoje só 2 memoriais de teste, sem placa real produzida, mas a janela pra fazer de graça fecha na primeira impressão de verdade.
- **Portal da Família (`/familia/[slug]`) ainda não tem nada de privacidade na tela** — só Central e Portal do Parceiro conseguem ver/trocar o modo hoje. A API (`/api/memorial-emails-autorizados`, `/api/memorial-visitantes`) já aceita sessão de família (mesmo tripé staff/parceiro-dono/família de `/api/memorial-storage-usage`), então dá pra ligar a UI lá depois sem mexer no backend — só falta a tela. Também em aberto (regra 15, não decidir sozinho): se a própria família deveria poder trocar o modo de privacidade sozinha, ou só visualizar — registrado em `docs/RASCUNHO_IDEIAS.md`.
