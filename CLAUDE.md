...
**Tamanho/qualidade (2026-07-14):** "isso é a marca, tem que ficar impecável sempre" — nunca deixar a logo pequena/discreta demais. Tamanhos mínimos usados hoje: `h-14` (sidebar/Navbar), `h-20`/`h-24` (telas de login, centralizadas), `h-16` (footer da landing). Arquivo fonte é 803×389px — segura bem esses tamanhos sem borrar; se precisar de exibição ainda maior no futuro, fazer upscale de verdade antes (Real-ESRGAN, grátis, open source — tentativa nessa sessão não completou por Space do Hugging Face não responder, retomar depois se precisar).
...
- [x] **3D da landing — logo real girando (2026-07-14)** — depois de remover o Canvas (3 rodadas antes: brasas → constelação → constelação com bloom, "cheio de traço"), voltou como a **logo oficial em 3D de verdade**. Pipeline: `public/logo-legado-digital.png` (flat) → **TripoSR** (Stability AI + Tripo AI, open source, MIT, grátis) rodando no Space público do Hugging Face (`stabilityai-triposr.hf.space`) — upload via `POST /upload`, depois `/call/preprocess` e `/call/generate` (API REST do Gradio, sem browser, sem Python, só `curl`) — devolveu `.glb` real (622KB), salvo em `public/3d/logo-legado-digital.glb`. `components/Hero3D.tsx` carrega via `useGLTF` do `drei`, gira de verdade no eixo Y (`useFrame`, não CSS), material metálico dourado (`MeshStandardMaterial`), luz + bloom real via `@react-three/postprocessing`. Sem custo — TripoSR não tem versão paga, só a API do Meshy (outro serviço testado antes) que era paga.
